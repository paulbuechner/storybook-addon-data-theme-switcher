...
export function getPrData(pr: PullRequest): PrData | undefined {
  ...
  const match = content.match(
    /## storybook-addon-data-theme-switcher@(?<version>\d.+)/
  );
  ...
  const body = [
    ...
    `description: Explore the changelog for storybook-addon-data-theme-switcher version ${version}. Learn about the latest features, bug fixes, and improvements.`,
...
export async function getPrByNumber(num: number): Promise<PullRequest> {
  const { data } = await octokit.pulls.get({
    owner: "paulbuechner",
    repo: "storybook-addon-data-theme-switcher",
    pull_number: num,
  });
...
export async function getMergedPrs(): Promise<PullRequests> {
  const { data } = await octokit.pulls.list({
    ...
    repo: "storybook-addon-data-theme-switcher",
    base: "main",
    head: "storybook-addon-data-theme-switcher:changeset-release/main",
    per_page: 100,
  });
